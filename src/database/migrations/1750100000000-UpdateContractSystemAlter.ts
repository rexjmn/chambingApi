// src/database/migrations/1750100000000-UpdateContractSystemAlter.ts
import { MigrationInterface, QueryRunner } from 'typeorm';

export class UpdateContractSystemAlter1750100000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // Renombrar metodo_pago_acordado a metodo_pago
    await queryRunner.query(`
      ALTER TABLE contratos 
      RENAME COLUMN metodo_pago_acordado TO metodo_pago
    `);

    // Ajustar valor por defecto de metodo_pago si es necesario
    await queryRunner.query(`
      ALTER TABLE contratos 
    ...
    // Renombrar porcentaje_comision a comision_plataforma
    await queryRunner.query(`
      ALTER TABLE contratos 
      RENAME COLUMN porcentaje_comision TO comision_plataforma
    `);

    // Ajustar tipo de comision_plataforma
    await queryRunner.query(`
      ALTER TABLE contratos 
      ALTER COLUMN comision_plataforma TYPE DECIMAL(5,2)
    `);

    // Calcular y agregar monto_trabajador basado en datos existentes
    await queryRunner.query(`
      ALTER TABLE contratos 
      ADD COLUMN IF NOT EXISTS monto_trabajador DECIMAL(10,2) DEFAULT 0
    `);

    // Calcular monto_trabajador para registros existentes
    // (monto_total - comisión)
    await queryRunner.query(`
      UPDATE contratos 
    ...
    // Eliminar columna pago_procesado si existe (ya no la necesitamos)
    await queryRunner.query(`
      ALTER TABLE contratos 
      DROP COLUMN IF EXISTS pago_procesado
    `);

    // Ajustar el campo estado para nuevos valores
    await queryRunner.query(`
      ALTER TABLE contratos 
    ...
    // ========== ACTUALIZAR TABLA estados_contrato ==========

    // Permitir NULL en estado_anterior
    await queryRunner.query(`
    ...
    // ========== GENERAR PINs PARA CONTRATOS EXISTENTES ==========
    // Los contratos existentes tendrán un PIN genérico
    // En producción, deberías generar PINs únicos
    await queryRunner.query(`
      UPDATE contratos 
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    // Revertir cambios (opcional, para desarrollo)
